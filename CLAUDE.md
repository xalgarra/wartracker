...
Event delegation: HTML dinámico usa `data-action` / `data-mini-id`. Listeners en contenedores estáticos con `e.target.closest('[data-action]')`. Sin `window.*`.
...
- SQL: nunca ejecutar directamente — crear archivo `sql/NN_nombre.sql` y el usuario lo ejecuta en Supabase.
